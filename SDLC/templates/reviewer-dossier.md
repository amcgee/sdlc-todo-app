...
The driver (`.claude/commands/sdlc.md` §6) posts this as the merge-ready PR comment — the human
...
- **Adversarial record**: `<details><summary>Findings &amp; outcomes</summary>…</details>` — each
  ...
  "No findings" is one line. Include the pm's design-conformance note verbatim.
- **Second-opinion leads**: one line per solicited (Copilot) review thread → its disposition
  (fixed via <F-id> / covered by <test> / rebutted — mirroring the §6.2 replies), or
...
- **Spec economy**: run `python SDLC/scripts/spec-ratio.py --item ISSUE-<n> --base origin/<default>`
  and quote its line (spec+design vs code, the ratio). Record it on the ledger so `metrics` shows
  it: `sdlc.py note --item ISSUE-<n> --by architect --msg "spec economy: <the ratio line>"`. A high
  ratio is a review prompt (duplication / implementation-transcription / design re-derivation),
  never a gate.
