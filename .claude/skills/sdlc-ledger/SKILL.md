...
| `attest --ref <ITEM>-F# --by verifier --file path --msg "…"` | verifier | prove an **artifact-only** fix (tests/docs/comments — no product code, so no fail→pass test can exist); names every file touched, CI confirms each is outside `shipped_paths` |
